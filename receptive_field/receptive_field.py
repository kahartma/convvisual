...
def make_and_save_RF_data(modelpath,savepath,classes,layer_indeces,
	...
	for i,layer_ind in enumerate(layer_indeces):
		...
		joblib.dump(RFELayer,os.path.join(layerpath,'RF_layer.data'),compress=True)
		for cl in classes:
			usplit = 1
			if unit_batch_sizes is not None:
				usplit = unit_batch_sizes[i]
			RFEClass = ReceptiveFieldExtractorClass(RFEData,RFELayer,cl)
			...
			for filter_ind in RFEClass.max_filters:
				RFEFilter = ReceptiveFieldExtractorFilter(RFEData,RFELayer,RFEClass,filter_ind,unit_batch_size=usplit)

				filterpath = os.path.join(classpath,'Filter_%d'%filter_ind)
				if not os.path.isdir(filterpath):
					os.makedirs (filterpath)
				joblib.dump(RFEFilter,os.path.join(filterpath,'RF_filter.data'),compress=True)
...
def load_LayerData(savepath,layer_ind):
	return joblib.load(os.path.join(savepath,'Layer_%02d'%layer_ind,'RF_layer.data'))

def load_ClassData(savepath,layer_ind,cl):
	return joblib.load(os.path.join(savepath,'Layer_%02d'%layer_ind,'Class_%s'%str(cl),'RF_class.data'))

def load_FilterData(savepath,layer_ind,cl,filter_ind):
	return joblib.load(os.path.join(savepath,'Layer_%02d'%layer_ind,'Class_%s'%str(cl),'Filter_%d'%filter_ind,'RF_filter.data'))
...
class ReceptiveFieldExtractorClass:
	...
	def __init__(self,ExtractorData,ExtractorLayer,cl):
...
class ReceptiveFieldExtractorFilter:
	def __init__(self,ExtractorData,ExtractorLayer,ExtractorClass,filter_ind,unit_batch_size=1):
		cl = ExtractorClass.cl
		if cl is not None:
			outputs = ExtractorLayer.outputs[ExtractorClass.input_indeces]
			inputs = ExtractorData.inputs[ExtractorClass.input_indeces]
		else:
			outputs = ExtractorLayer.outputs
			inputs = ExtractorData.inputs
		...
		print 'Outputs',outputs.shape,filter_ind
		tmp_units = get_most_active_units_in_layer_from_output(outputs[:,filter_ind:filter_ind+1,:,:],
										None,0,
										n_units=ExtractorData.nUnits,
										abs_act=False).astype(np.uint32)

		tmp_units[:,1] = filter_ind
		...
		self.max_units_in_filters = np.asarray(self.max_units_in_filters,dtype=np.uint32)
