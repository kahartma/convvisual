...
def make_and_save_RF_data(modelpath,savepath,classes,layer_indeces,
	...
	for i,layer_ind in enumerate(layer_indeces):
		...
		for cl in classes:
			...
			for filter in RFEClass.max_filters:
				RFELayer = ReceptiveFieldExtractorLayer(RFEData,RFELayer,RFEClass,unit_batch_size=usplit)

				filterpath = os.path.join(classpath,'Filter_%d'%filter)
...
class ReceptiveFieldExtractorFilter:
	def __init__(self,ExtractorData,ExtractorLayer,ExtractorClass,filter,unit_batch_size=1):
		...
		self.max_units_in_filter = list()
		print 'Outputs',outputs.shape
		tmp_units = get_most_active_units_in_layer_from_output(outputs[:,filter,:,:],
